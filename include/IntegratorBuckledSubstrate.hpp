#ifndef INTEGRATORBUCKLEDSUBSTRATE_HPP
#define INTEGRATORBUCKLEDSUBSTRATE_HPP
...
class IntegratorBuckledSubstrate : public IntegratorBase
{

public:
		
	IntegratorBuckledSubstrate(CSimulation * sim_);
		 
	~IntegratorBuckledSubstrate();
...
private:
	...
	void temperatureInteraction
			(double (&tempForce_)[3]);													// Calculates the force due to the thermostat				

	void ApplyMaxVelocities
			(CParticle * p_,
			 double &velx_,
			 double & vely_,
			 double & velz_);												// Apply max velocities
