...
double BesselsForce(const double & dist_, CParallelEulerFMAIntegrator *integrator_);
...
void CParallelEulerFMAIntegrator::Initialise()
{	
...
CParallelEulerFMAIntegrator::~CParallelEulerFMAIntegrator()
{
...
void CParallelEulerFMAIntegrator::Integrate()
{
	...
	cilk_for(int i = 1; i < MAXLINKEDLISTSIZE-1; ++i)
	{
		for(int j = 1; j < MAXLINKEDLISTSIZE-1; ++j)
		{
									...
									p != (*cll[i][j].get_cellList()).end(); ++p)
				{
					...
					double accelx=(forcep_dx + forcep_tx);
					double accely=(forcep_dy + forcep_ty);
					
					double velx = p->get_velx() + accelx*dt;
					double vely = p->get_vely() + accely*dt;
	...
	// Apply Velocity Rescaling to ensure energy conservation
	VelocityRescaling(vorticesList);
...
void CParallelEulerFMAIntegrator::CreateCellLinkedLists( CCell ** cll_, std::list<CParticle> * list_ )
{
...
void CParallelEulerFMAIntegrator::CopyCellLinkedList( CCell ** cllsource_, CCell ** clltarget_)
{
...
void CParallelEulerFMAIntegrator::vvInteration(
...
double CParallelEulerFMAIntegrator::forceForm(double dist_, bool inbath_)
{
	double force=0;
...
void CParallelEulerFMAIntegrator::CellLinkedListToList(CCell** cll_,std::list<CParticle> * vorticesList_)
{
	for(int i = 0; i < MAXLINKEDLISTSIZE; ++i)
	{
		for(int j = 0; j < MAXLINKEDLISTSIZE; ++j)
...
int CParallelEulerFMAIntegrator::what_icell(std::list<CParticle>::iterator a_) const
{
...
int CParallelEulerFMAIntegrator::what_jcell(std::list<CParticle>::iterator a_) const
{
...
double CParallelEulerFMAIntegrator::AndersonTS() const
{
	double p=dt/tau;
...
double CParallelEulerFMAIntegrator::LindemanTS() const
{
...
void CParallelEulerFMAIntegrator::temperatureInteraction(double (&tempForce_)[2])
{
...
void CParallelEulerFMAIntegrator::ApplyMaxVelocities(std::list<CParticle>::iterator p_, double &velx_, double & vely_)
{		
...
void CParallelEulerFMAIntegrator::CheckDouble(double num_, const std::string & varname_, const std::string & source_)
{
	if (num_!=num_)
	{
		std::stringstream oss;
...
void CParallelEulerFMAIntegrator::CheckDuplicatePositions(CCell **cell_)
{
...
void CParallelEulerFMAIntegrator::ClearLinkedLists()
{
	for(int i = 0; i < MAXLINKEDLISTSIZE; ++i)
	{
		for(int j = 0; j < MAXLINKEDLISTSIZE; ++j)
...
double CParallelEulerFMAIntegrator::GetM2Average() const {return M2Sum/sim->get_t();}
	
double CParallelEulerFMAIntegrator::GetM2FullAverage() const {return M2FullSum/sim->get_t();}

void CParallelEulerFMAIntegrator::VelocityRescaling(std::list<CParticle> * vorticesList)
{
	...
	for(std::list<CParticle>::iterator p = vorticesList->begin();
			p!=vorticesList->end(); ++p)
	{
		double velx= p->get_velx();
		double vely= p->get_vely();
		double modv2= velx*velx+vely*vely;
		
		FrameEnergy+=0.5*modv2;
		
	}
	
	static double Energy0 = FrameEnergy;
	
	// sum 0.5* velx0*velx0+vely0*vely0  = alpha^2 sum 0.5* velx*velx +  vely*vely
	// E0/E=alpha^2
	// rescale all velocities by alpha=Sqrt[E0/E]
	
	double alpha = sqrt(Energy0/FrameEnergy);
