...
double BesselsForce(const double & dist_, IntegratorBuckledSubstrate *integrator_);


IntegratorBuckledSubstrate::IntegratorBuckledSubstrate(CSimulation *sim_)
{
	M2=0;
	M2Full=0;
	M2Sum=0;
...
void IntegratorBuckledSubstrate::Initialise()
{	
	...
	sim->ReadVariableFromBatchFile(springK, "Header.springK");  
...
IntegratorBuckledSubstrate::~IntegratorBuckledSubstrate()
{
...
void IntegratorBuckledSubstrate::Integrate()
{
	...
	cilk_for (long ii = 0; ii != vorticesVector.size(); ++ii)
	{
		...
		double vortexForce[3]={0,0,0};
		double pinsForce[3]={0,0,0};
		double disorderForce[3]={0,0,0};
		double tempForce[3]={0,0,0};
		
		// calculate forces due to temperature kick 
		thermostats::Andersen3D(temp, kB, eta, dt, tau, tempForce);
		...
		p != vorticesList->end(); ++p )
	{
		...
		double forcep_dz= p->get_force_dz();
		
		double forcep_tx= p->get_force_tx();
		double forcep_ty= p->get_force_ty();
		double forcep_tz= p->get_force_tz();
		
		
		frame_force_d += sqrt(forcep_dx*forcep_dx+forcep_dy*forcep_dy+forcep_dz*forcep_dz);
		frame_force_t += sqrt(forcep_tx*forcep_tx+forcep_ty*forcep_ty+forcep_tz*forcep_tz);
		
		M2 += (forcep_tx*dt/eta)*(forcep_tx*dt/eta)+(forcep_ty*dt/eta)*(forcep_ty*dt/eta)+(forcep_tz*dt/eta)*(forcep_tz*dt/eta);
		M2Full += (p->get_x()-p->get_lastx())*(p->get_x()-p->get_lastx())+(p->get_y()-p->get_lasty())*(p->get_y()-p->get_lasty())+(p->get_z()-p->get_lastz())*(p->get_z()-p->get_lastz());
...
void IntegratorBuckledSubstrate::CreateCellLinkedLists( CCell ** cll_, std::list<CParticle> * list_ )
{
...
void IntegratorBuckledSubstrate::CopyCellLinkedList( CCell ** cllsource_, CCell ** clltarget_)
{
...
void IntegratorBuckledSubstrate::vvInteration(
		CParticle * p_,
		std::list<CParticle> & cell_,  
		double (&force_)[3],
		boost::function<double (double, IntegratorBuckledSubstrate *)> func_
		)
{
	...
	*/
									...
									q != cell_.end(); ++q)
	{
		...
		double rsq = pxqx*pxqx + pyqy*pyqy +pzqz*pzqz;
		...
		double rhat[3] ={pxqx*oneoverr,pyqy*oneoverr,pzqz*oneoverr};
...
double IntegratorBuckledSubstrate::forceForm(double dist_, bool inbath_)
{
	double force=0;
...
void IntegratorBuckledSubstrate::CellLinkedListToList(CCell** cll_,std::list<CParticle> * vorticesList_)
{
	for(int i = 0; i < MAXLINKEDLISTSIZE; ++i)
	{
		for(int j = 0; j < MAXLINKEDLISTSIZE; ++j)
...
int IntegratorBuckledSubstrate::what_icell(std::list<CParticle>::iterator a_) const
{
...
int IntegratorBuckledSubstrate::what_jcell(std::list<CParticle>::iterator a_) const
{
...
int IntegratorBuckledSubstrate::what_icell(CParticle * a_) const
{
	double i= floor((a_->get_x()-sim->get_xlo()+2*cellSize)/cellSize);
	//std::cout << i << std::endl;
	if (i!=i)
...
int IntegratorBuckledSubstrate::what_jcell(CParticle *a_) const
{
	double j= floor((a_->get_y()-sim->get_ylo()+2*cellSize)/cellSize);
	if (j!=j)
	{
...
void IntegratorBuckledSubstrate::ApplyMaxVelocities(CParticle * p_, double &velx_, double & vely_, double & velz_)
{		
...
void IntegratorBuckledSubstrate::CheckDouble(double num_, const std::string & varname_, const std::string & source_)
{
	if (num_!=num_)
	{
		std::stringstream oss;
...
void IntegratorBuckledSubstrate::CheckDuplicatePositions(CCell **cell_)
{
...
void IntegratorBuckledSubstrate::ClearLinkedLists()
{
	for(int i = 0; i < MAXLINKEDLISTSIZE; ++i)
	{
		for(int j = 0; j < MAXLINKEDLISTSIZE; ++j)
...
double IntegratorBuckledSubstrate::GetM2Average() const {return M2Sum/sim->get_t();}
	
double IntegratorBuckledSubstrate::GetM2FullAverage() const {return M2FullSum/sim->get_t();}
