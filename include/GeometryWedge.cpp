...
GeometryWedge::GeometryWedge(CSimulation * sim_)
{
	sim=sim_;
...
void GeometryWedge::LoadBatchFile()
{
...
void GeometryWedge::InitialiseGeometry()
{
	InitialiseFiles();
	LoadBatchFile();
...
void GeometryWedge::InitialiseParameters()
{
	...
	*/
	...
	//removewedgey1=removewedgey0+(bathLength-removewedgex0)*tan(M_PI/6);
	//removewedgey2=removewedgey0-(bathLength-removewedgex0)*tan(M_PI/6);
...
void GeometryWedge::InitialiseVortices()
{
...
void GeometryWedge::InitialiseRandomMobileParticles()
{         
...
void GeometryWedge::ReplaceEscapedVortices()
{
...
void GeometryWedge::InitialiseCEParticles()
{
	double locala0=a0;
	double localb0=b0;
	...
	double r1[2]={a0*cos(M_PI/6),a0*sin(M_PI/6)};
	double r2[2]={0,a0};
	
	int jmin = -ceil( (bathLength*sin(M_PI/6)+6*b0)/r2[1] );
...
void GeometryWedge::AddParticlesForDT(std::list<CParticle> & iList)
{
	// Add A particles
	iList.clear();
...
double GeometryWedge::GetRemovalSourceX() const
{
	return 5*a0;
} 

double GeometryWedge::GetRemovalSinkX() const
{
	return 2*bathLength+channelLength - 5*a0;
} 

void GeometryWedge::UpdateBathDensities()
{
	int t = sim->get_t();
...
bool GeometryWedge::AddParticleToBath(std::string location_)
{
		int t = sim->get_t();
...
bool GeometryWedge::RemoveParticleFromBath(std::string location_)
{
		int t = sim->get_t();
...
double GeometryWedge::CalcSinkB() const
{
...
double GeometryWedge::CalcSourceB() const
{
	
	/*double aaverage=0;
	int numa=0;
	
	//if (triangulatedLinesList->size() == 0) throw std::runtime_error("GeometryWedge::CalcSourceB() No triangulated particles");
	for (std::list<CDelLine>::iterator p = triangulatedLinesList->begin();
				p!=triangulatedLinesList->end(); ++p)
	{
...
void GeometryWedge::PerStepAnalysis()
{
	  OutputParticlePositions(); 
	  CalculateAndOutputAvVel();
}

void GeometryWedge::EndofSimAnalysis()
{
...
void GeometryWedge::PerStepUpdates()
{
	UpdateBathDensities();
...
void GeometryWedge::CalculateAndOutputAvVel()
{
	/*
...
void GeometryWedge::OutputFinalParticlePositions()
{
...
void GeometryWedge::OutputParticlePositions()
{
	std::stringstream oss;
...
void GeometryWedge::OutputAverages()
{	
	int t = sim->get_t();
	if (sim->get_simulation_time()+1!=t) throw std::runtime_error("GeometryWedge::OutputAverages() Averages must be output at the end of the simulation.");
...
void GeometryWedge::CalculateAndOutputNd()
{
...
std::list<CParticle> * GeometryWedge::GetIParticles()
{
	return AParticlesList;
}

void GeometryWedge::GetJParticles(std::list<CParticle> & jList)
{
	// Add A particles
	jList.clear();
...
void GeometryWedge::InitialiseFiles()
{
...
void GeometryWedge::WrapVorticesY(std::list<CParticle>& jList)
{
...
void GeometryWedge::DoWrapY(std::list<CParticle>::iterator p, std::list<CParticle>& jList)
{
...
bool GeometryWedge::etchParticleWedge(std::list<CParticle>::iterator p_)
{
	double x = p_->get_x();
	double y = p_->get_y();
...
bool GeometryWedge::removeParticleWedge(std::list<CParticle>::iterator p_)
{
	double x = p_->get_x();
	double y = p_->get_y();
